...
get_data
	...
	MOV r5, #0 ;filter initializing
	mov r12, #480
	add r12, r12, #2
	cmp r3, r12 ;; check r3 > 480
	BEQ setlist
	mov r4, #4
	sub r10, r3, #5
	mul r4, r10, r4
	mov r12, r4
	ldr r4, [r0,r4] ;get sampled address start
	ADD r0, r0, r12
	MOV r2, #0 ;initialize
	MOV r6, #0;
	MOV r7, #0;
...
get_nextdata
	MOV r2, r7; save mul result
	ldr r4, [r0,#4] ;get next sample data
	ADD r0, r0, #4
...
get_filter
	...
	cmp r5, #0
	BGT get_nextfilter
	ldr r1, =Filter ;filter address	
	ldr r6, [r1], #4
	B multiplier
get_nextfilter
	ldr r6, [r1] ;get nextfilter value
	ADD r1, r1,#4
	B multiplier
	
multiplier
	MOV r7, r4, lsr #31 ;r7 is sampledata sign bit
	MOV r8, r6, lsr #31 ;r8 is filter sign bit
	eor r7, r7, r8 ;xor sigh bit
	MOV r7, r7, lsl #31 ;set sign bit
	
	MOV r8, r4, lsl #1
	MOV r8, r8, lsr #24
	MOV r9, r6, lsl #1
	MOV r9, r9, lsr #24 ;get exponent
	ADD r8, r8, r9
	SUB r8, #254
	ADD r8, #127
	MOV r8, r8, lsl #23 ;set exponent
	ADD r7, r8
	...
	MOV r9, r6, lsl #9
	MOV r9, r9, lsr #9
	ADD r8, r8, #0x00800000
	ADD r9, r9, #0x00800000
	UMULL r11, r10, r8, r9
	CMP r10, #0x00008000 ;normarizing
	MOVGE r10, r10, lsl #8
	ADDGE r7, #0x00800000
	MOVLT r10, r10, lsl #9
	MOVGE r11, r11, lsr #24
	MOVLT r11, r11, lsr #23
...
Add_
	STMFA r14!,{r0-r1, r3-r6, r12} ;save value
	...
	CMP r2, #0x80000000; check sign bit
	MOVLS r0, #0
	MOVHI r0, #1
	...
	CMP r3,r4
	SUBGT r8, r3, r4 ;if r3 > r4
	SUBLT r8, r4, r3 ;if r4 > r3
	
	MOVGT r6, r6, LSR r8 ;shift mantissa
	MOVLT r5, r5, LSR r8
	
	MOVGT r9, r3
	MOVLE r9, r4
	
	CMP r0, r1 ;check sign bit
	BEQ same
	BNE dif
	
same
	ADD r5, r5, r6 ;add mantissa
	CMP r5, #0x01000000
	MOVGT r5, r5, LSR #1
	ADDGT r9, r9, #1
	B finish_sum
...
dif
	CMP r6, r5 ; compare mantissa
	SUBGT r5, r6, r5 ;if r6>r7
	SUBLT r5, r5, r6 ;if r7>r6
	MOVGT r0, r1
	MOVLT r0, r0
	MOV r8, #0

normalize
	CMP  r5,#0x00800000 ;if r5 < 1.~
	MOVLT r5, r5, LSL #1
	ADDLT r12, r12, #1
	BLT normalize
	SUBGE r9, r9, r12 ;exponent
...
finish_sum
	...
	MOV r9, r9, LSL #23
	ADD r7, r0, r9
	ADD r7, r7, r5 ;result add

	ldmfa r14!,{r0-r1, r3-r6, r12} ;;get saved  value
	
	cmp r5, #6 ;if mul 6 time go to add
	BEQ savedata
	B get_nextdata
...
savedata 
	...
	CMP r9, r10 ;if save value < -4.7
	BLT pushdata
	
	B get_data
	
pushdata
	STMEA r13!,{r7} ;push data
	SUB r8, r3, #5
	STMEA r13!,{r8} ;push index
	B get_data
	
setlist
	ldr r0, Tempb ; all sample data cal finish
	ldr r1, Save_Result_Address
	ldr r2, Tempc
	
loop_set
	ldmfd r0!,{r3} ;save result
	CMP r3, #0
	LDREQ r2, Tempc
	BEQ set_index
	str r3,[r1],#4
	ldmfd r0!,{r3}
	strh r3,[r2],#2 ;save index different memory
	B loop_set
	
set_index
	ldmfd r2!,{r3} ;and get index
	cmp r3, #0
	BEQ finish_program
	str r3,[r1],#4 ;push index data
	B set_index
...
Result ; Store less that -4.7 of the convolution results(32bit, double word) and their indices(word).
	DCD 0xC0966666 ; result -4.7
	
Filter
	DCD 0xBE589375, 0xBF4D5CFB, 0xBFA39581, 0xBEDCAC08, 0x3FDC28F6, 0x3FF9999A ; 6filter
		;-0.215, 	-0.8022 ,	 -1.278,	-0.431,		1.72		1.95
